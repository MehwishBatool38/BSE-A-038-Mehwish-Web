...
const Profiles = {
    ...
    async setScholarship(studentId, percentage) {
        return this.update(studentId, { scholarship_percentage: percentage });
    },

    async setWaiver(studentId, waiver) {
        return this.update(studentId, { waiver: waiver });
    },
    ...
    async getFinancialStatus(studentId) {
        ...
        return {
            ...
            waiver: user.waiver || false,
            total_fines: user.total_fines || 0
        };
    },
...
const FeeAssignments = {
    ...
    async getForStudent(studentId) {
        ...
        const plans = DemoStorage.getFeePlans();
        const users = DemoStorage.getUsers();
        const found = assignments.filter(a => a.student_id === studentId && !a.deleted);
        return found.map(a => ({
            ...a,
            fee_plans: plans.find(p => p.id === a.plan_id),
            profiles: users.find(u => u.id === a.student_id)
...
const Fines = {
    async getAll() {
        return JSON.parse(localStorage.getItem('edufee_fines') || '[]');
    },

    async getForStudent(studentId) {
        const fines = await this.getAll();
        return fines.filter(f => f.student_id === studentId);
    },

    async create({ studentId, assignmentId, installmentNo, amount, reason }) {
        const fines = await this.getAll();
        const newFine = {
            id: Math.max(...fines.map(f => f.id), 0) + 1,
            student_id: studentId,
            assignment_id: assignmentId,
            installment_no: installmentNo,
            amount: amount,
        ...
        fines.push(newFine);
        localStorage.setItem('edufee_fines', JSON.stringify(fines));
        return newFine;
    },
    ...
    async calculateAndApplyFines() {
        ...
        for (const assignment of assignments) {
            ...
            for (const installment of plan.installments) {
                ...
                const existingFine = (await this.getAll()).find(f => f.student_id === assignment.student_id && f.assignment_id === assignment.id && f.installment_no === installment.no);
        ...
        const applied = [];
        for (const fine of finesToApply) {
            applied.push(await this.create(fine));
            // Update total_fines
            const user = users.find(u => u.id === fine.student_id);
            if (user) {
                user.total_fines = (user.total_fines || 0) + fine.amount;
                DemoStorage.saveUsers(users);
            }
        }
        return applied;
